...
bool _isValidBase64(String str) {
  ...
  Future<void> _loadReactions() async {
    ...
    if (mounted) setState(() => _reactions.addAll(map));
  }
  ...
  Map<String, dynamic> _decodeMessage(Map<String, dynamic> msg) {
    ...
    if (msg['media_url'] != null) return {...msg, 'decrypted_content': null};
    try {
  ...
  Future<void> _sendMessage() async {
    ...
    if (_aesKey == null) {
      Get.snackbar('Error', 'Encryption key not ready. Please wait.');
      return;
    }
    if (_conversationId == null) {
      Get.snackbar('Error', 'Conversation ID missing.');
      return;
    }
    ...
    try {
      final inserted = await _supabase
          ...
          .insert({
            ...
            'sender_id': _currentUserId,
            'encrypted_content': enc.content,
            'iv': enc.iv,  'created_at': DateTime.now().toUtc().toIso8601String(), // ✅ add this
...
Future<void> _ensureConversationExists() async {
  ...
  Future<void> _toggleReaction(String msgId, String emoji) async {
    ...
    if (existing != null) {
      ...
      final inserted = await _supabase
          ...
          .select()
          .single();
      setState(() => _reactions.putIfAbsent(msgId, () => []).add(inserted));
    }
  }
  ...
  Widget build(BuildContext context) {
      print(' ChatScreen build: canSendMessages = ${widget.canSendMessages}');
  ...
  Widget _buildInputArea() {
    if (!widget.canSendMessages) {
      return Container(
        ...
        child: Center(
          child: Text(
            'Messaging is only allowed on the day of your appointment.',
